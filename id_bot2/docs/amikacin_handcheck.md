# Amikacin — Clinical Hand-Check Sheet
...
|------|------|------|-------|
| NORMAL | 25 mg/kg once, round up | GFR any; no renal dose reduction | — |
| IHD | do not use | IHD | — |
...
## 4. Selection ladder
  ...
  1. `if ihd` → tier `IHD`
  2. `if crrt` → tier `NORMAL`
  3. `if gfr >= 0` → tier `NORMAL`
  4. `default` → `DEFAULT_ANSWER`
