# Fluconazole — Clinical Hand-Check Sheet

**Protocol ID:** `fluconazole`
...
`fluconazole`, `fluconazol`, `fluconazole dose`, `fluconazole dosing`
...
|------|------|------|-------|
| LOADING *(always_show)* | 800 mg once | day 1, then start maintenance day 2 | — |
| NORMAL | 800 mg once day 1, then 1 x 400 mg | GFR >=10 | — |
| GFR_LT_10 | 800 mg once day 1, then 1 x 200 mg | GFR <10 | — |
| IHD | 800 mg once day 1, then 1 x 400 mg | IHD | — |
| CRRT | 800 mg once day 1, then 1 x 400 mg | CRRT | — |
...
## 4. Selection ladder
  ...
  1. `if ihd` → tier `IHD`
  2. `if crrt` → tier `CRRT`
  3. `if gfr >= 10` → tier `NORMAL`
  4. `if gfr < 10` → tier `GFR_LT_10`
