# Ciprofloxacin — Clinical Hand-Check Sheet

**Protocol ID:** `ciprofloxacin`
...
`ciprofloxacin`, `cipro`, `ciprofloxacin dose`, `ciprofloxacin dosing`
...
|------|------|------|-------|
| NORMAL | 3 x 400 mg IV or 2 x 500 mg PO | GFR >=60 | — |
| GFR_30_TO_60 | 2 x 400 mg IV | GFR 30-60 | — |
| GFR_LT_30 | 1 x 400 mg IV | GFR <30 | — |
| IHD | 1 x 400 mg IV | IHD | — |
| CRRT | 2 x 400 mg IV | CRRT | — |
...
## 4. Selection ladder
  ...
  1. `if ihd` → tier `IHD`
  2. `if crrt` → tier `CRRT`
  3. `if gfr >= 60` → tier `NORMAL`
  4. `if gfr >= 30 and gfr < 60` → tier `GFR_30_TO_60`
  5. `if gfr < 30` → tier `GFR_LT_30`
