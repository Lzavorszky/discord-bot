# ceftolozane/tazobactam — Clinical Hand-Check Sheet

**Protocol ID:** `ceftolozane_tazobactam`
**Source:** uploaded antibiotic renal dosing DOCX - ceftolozane/tazobactam
**YAML version:** 0.1
...
`ceftolozane/tazobactam`, `ceftolozane tazobactam`, `ceftolozane-tazobactam`, `zerbaxa`, `ceftolo tazo`, `ceftolozane/tazo`
...
|------|------|------|-------|
| LOADING *(always_show)* | 2/1 g once | start of therapy | start continuous infusion immediately |
| NORMAL_GFR_GE_50 | 9 g/day | GFR >=50 | 1/0.5 g/50 mL, 12.5 mL/h |
| NORMAL_GFR_20_50 | 4.5 g/day | GFR 20-50 | 1/0.5 g/50 mL, 6.3 mL/h |
| SEVERE_AKI | 3 g/day | GFR <20 | 1/0.5 g/50 mL, 4.2 mL/h |
| ANURIA_IHD | 1.5 g/day | anuria or IHD | 1/0.5 g/50 mL, 2.1 mL/h |
| CRRT | 4.5 g/day | CRRT | 1/0.5 g/50 mL, 12.5 mL/h |

**Verify each row against source `uploaded antibiotic renal dosing DOCX - ceftolozane/tazobactam`:**
...
## 4. Selection ladder
  ...
  1. `if ihd` → tier `ANURIA_IHD`
  2. `if crrt` → tier `CRRT`
  3. `if gfr >= 50` → tier `NORMAL_GFR_GE_50`
  4. `if gfr >= 20 and gfr < 50` → tier `NORMAL_GFR_20_50`
