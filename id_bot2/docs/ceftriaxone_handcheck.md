# Ceftriaxone — Clinical Hand-Check Sheet

**Protocol ID:** `ceftriaxone`
...
`ceftriaxone`, `ceftriaxon`, `ceftriaxone dose`, `ceftriaxone dosing`
...
|------|------|------|-------|
| LOADING *(always_show)* | 2 g IV bolus once | start of therapy, then continue scheduled dosing | — |
| NORMAL | 1 x 2 g IV bolus | any renal or liver function | — |
| HIGH_DOSE | 2 x 2 g IV bolus | CNS infection, septic shock, or albumin <30 g/L | — |
...
## 4. Selection ladder
  ...
  1. `if cns_infection or septic_shock or hypoalbuminemia` → tier `HIGH_DOSE`
  2. `if gfr >= 0` → tier `NORMAL`
  3. `default` → `DEFAULT_ANSWER`
...
**Footer:** No dose reduction is required for renal or liver dysfunction. Use high-dose 2 x 2 g IV bolus for CNS infection, septic shock, or albumin <30 g/L.
