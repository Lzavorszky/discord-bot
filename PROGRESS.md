...
- **`./check.sh`:** id_bot2 unit tests ✓ (**119 passed**, 1 live-skipped — incl. 31 new router tests), protocol schema + linter ✓ (**29** valid records after relebactam removal, no alias collisions), LLMProvider contract ✓ (11 passed, 1 live-skipped), regression harness offline ✓ (144 cases valid). `--target new` ✓ **125 PASS / 0 FAIL / 19 SKIP** (now with the router input→call cross-check). Legacy suite 330/331 (one pre-existing env failure, noted below).
